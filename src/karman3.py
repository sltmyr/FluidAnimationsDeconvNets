...
for yy in range(8,24):
	for xx in range(10,41):
		for rr in range(4,9):
			...
			obstacle  = Cylinder( parent=s, center=gs*vec3(xx/64.0,yy/32.0,0.5), radius=rr, z=gs*vec3(0, 0, 1.0))
			phiObs    = obstacle.computeLevelset()

			# slightly larger copy for density source
			densInflow  = Cylinder( parent=s, center=gs*vec3(xx/64.0,yy/32.0,0.5), radius=rr, z=gs*vec3(0, 0, 1.0))
